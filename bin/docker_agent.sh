...
# Use by sourcing script and running bash function docker_agent_activate

__docker_agent_print_usage() {
	usage="$(basename "$0") [ -c | -h | -i | -k | -l | -p | -r | -s | -a ssh-key] -- Control docker ssh agent
...
# Variables:
# DOCKER_SSH_AUTH_SOCK: The docker ssh-agent socket file
...
__docker_agent_print_agent_status () {
	local silent=$1
	__docker_agent_check_agent_running
	running=$?
	if [ "$silent" != "true" ]; then
...
__docker_agent_parse_args () {
	while getopts ":srklphica:" opt ${BASH_ARGV}; do
		case $opt in
			r)
				__docker_agent_start_docker_ssh_agent
				;;
			...
			l)
				if [ ! -e "${__DOCKER_AGENT_ENV}" ]; then
					...
					__docker_agent_load_docker_ssh_agent_env
					__docker_agent_print_agent_status $DOCKER_SSH_AGENT_LOAD_SILENT
					running=$?
